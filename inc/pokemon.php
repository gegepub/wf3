...
do {
  ...
  // attaque de pokemon1 contre pokemon2
  attaque($nom_pokemon1, $pokemon1, $nom_pokemon2, $pokemon2);
  if ($pokemon1['pv'] <= 0 || $pokemon2['pv'] <= 0) break;

  // contre attaque de pokemon2 contre pokemon1
  attaque($nom_pokemon2, $pokemon2, $nom_pokemon1, $pokemon1);

  } while ($pokemon1['pv'] > 0 && $pokemon2['pv'] > 0);
// équivalent à !($pikachu['pv'] <= 0 || $bulbizarre['pv'] <= 0)
